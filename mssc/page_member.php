	...
	if(!empty($_POST['email_recover'])) {
		...
		if(count($result) == 1) { 
			...
			if(count($result) == 1) {
				$found_email = true;
				$password = unique_id();
				$sql = "UPDATE mssc_user SET password = MD5('$password') WHERE id = " . $result[0]->id;
				$wpdb->get_results($wpdb->prepare($sql, 0));
			}
		}
