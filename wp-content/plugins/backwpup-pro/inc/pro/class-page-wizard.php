...
class BackWPup_Pro_Page_Wizard {
	...
	public static function save_wiz_data() {
		...
		if ( ! empty( self::$wiz_data ) )
			set_site_transient( 'BackWPup_Wiz_' . self::$wiz_data_id, self::$wiz_data, 3600 );
		else
	...
	public static function admin_print_styles() {
		/**
		 * @todo Transfer CSS in stylesheet file!
		 *
		 */
		?>
		<style type="text/css" media="screen">
			...
			#wizard_navigation ul li:last-child:after {
				content: "";
				display: none;
			}
	...
	public static function page() {
		...
		if ( empty( self::$wiz_data ) ) {
			...
			<div class="wrap" id="backwpup-page">
				<h2><span id="backwpup-page-icon">&nbsp;</span><?php echo sprintf( _x( '%s Wizards', 'Plugin Name', 'backwpup' ), BackWPup::get_plugin_data( 'name') ); ?></h2>
				<?php
				foreach ( $wizards as $wizard_class ) {
					...
					if ( ! current_user_can( $wizard_class->info[ 'cap' ] ) )
						continue;
					//get info of wizard
					echo '<div class="wizardbox post-box backwpup-floated-postbox" id="wizard-' . strtolower( $wizard_class->info[ 'ID' ] ) . '"><form method="get" action="' . network_admin_url( 'admin.php' ) . '">';
						...
						echo '<input type="hidden" name="wizard_start" value="' . $wizard_class->info[ 'ID' ] . '" />';
						echo '<div class="wizardbox_start"><input type="submit" name="submit" class="button button-primary button-primary-bwp" value="' . __( 'Start wizard', 'backwpup' ) . '" /></div>';
					echo '</form></div>';
				}
				?>
			</div>
		...
		//execute wizard
			...
			?>
			<div class="wrap" id="backwpup-page">
				<h2 id="wizard_name"><span id="backwpup-page-icon">&nbsp;</span><?php echo  sprintf( _x( '%s Wizard:', 'Plugin Name', 'backwpup' ), BackWPup::get_plugin_data( 'name') ) . ' ' . esc_html( $wizards[ self::$wiz_data[ 'wizard' ][ 'id' ] ]->info[ 'name' ] ); ?></h2>
                ...
                <form method="post" id="wizard_form" enctype="multipart/form-data" action="<?php echo admin_url( 'admin-post.php' ); ?>">
                    <input type="hidden" name="action" value="backwpup_wizard" />
					<input type="hidden" name="BackWPup_Wizard_ID" value="<?php echo self::$wiz_data_id; ?>" />
					<input type="hidden" name="nextstep" value="<?php echo esc_html( self::$wiz_data[ 'wizard' ][ 'step' ] ); ?>" />
					<?php
					wp_nonce_field( 'wizard' );
							...
							<ul>
								...
								foreach ($wizard_steps as $step ) {
									...
									if ( self::$wiz_data[ 'wizard' ][ 'step' ] != 'last' && ! $wizards[ self::$wiz_data[ 'wizard' ][ 'id' ] ]->is_step_by_step( self::$wiz_data ) )
										echo '<a class="wizardsteplink" href="' . network_admin_url( 'admin.php' ) . '?page=backwpupwizard&BackWPup_Wizard_ID='.self::$wiz_data_id .'&step=' . esc_attr( $step[ 'id' ] )  . '" data-nextstep="' . esc_attr( $step[ 'id' ] )  . '">';
								...
								if ( self::$wiz_data[ 'wizard' ][ 'step' ] == 'last' ) {
									...
									if ( $wizards[ self::$wiz_data[ 'wizard' ][ 'id' ] ]->is_step_by_step( self::$wiz_data ) )
										...
										echo '<li class="execute"><a class="wizardsteplink" href="' . network_admin_url( 'admin.php' ) . '?page=backwpupwizard&BackWPup_Wizard_ID='.self::$wiz_data_id .'&step=last" data-nextstep="last">' . esc_attr( $wizards[ self::$wiz_data[ 'wizard' ][ 'id' ] ]->get_last_button_name( self::$wiz_data ) ) . '</a></li>';
								}
								?>
							</ul>
						</div>
						...
						<div id="wizard_navbuttons">
							...
							if ( isset( self::$wiz_data[ 'wizard' ][ 'step' ]) && self::$wiz_data[ 'wizard' ][ 'step' ] != 'last' ) {
								if 	( $wizard_steps[0][ 'id' ] != self::$wiz_data[ 'wizard' ][ 'step' ] ) {
									echo '<input type="submit" id="wizard-button-previous" name="wizard_button" class="button button-bwp button-previous-bwp" value="' . __( '‹ Previous', 'backwpup' ) . '" tabindex="2" accesskey="b" />&nbsp;';
								}
								...
								if  ( $wizard_steps[ $highest_step_key ][ 'id' ] != self::$wiz_data[ 'wizard' ][ 'step' ] || ( isset( $wizard_steps[ $highest_step_key ][ 'create' ] ) && ! $wizard_steps[ $highest_step_key ][ 'create' ] ) ) {
									echo '<input type="submit" id="wizard-button-next" name="wizard_button" class="button button-bwp button-primary-bwp button-next-bwp" value="' . __( 'Next ›', 'backwpup' ) . '" tabindex="1" accesskey="n" />&nbsp;';
								}
								if  ( $wizard_steps[ $highest_step_key ][ 'id' ] == self::$wiz_data[ 'wizard' ][ 'step' ] && ! ( isset( $wizard_steps[ $highest_step_key ][ 'create' ] ) && ! $wizard_steps[ $highest_step_key ][ 'create' ] ) ) {
								...
								echo '<input type="submit" id="wizard-button-chancel" name="wizard_button" class="button button-bwp button-cancel-bwp" value="' . __( 'Cancel', 'backwpup' ) . '" tabindex="3" />';
							} else {
